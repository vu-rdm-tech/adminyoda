...
def _monthly_research_stats(
    folder, stats, start_year, end_year, include_revisions=True
):
    for year in range(start_year, end_year + 1):
        ...
        for month in range(1, 13):
            if month not in stats[year]:
                stats[year][month] = {}
            s = (
                ResearchStats.objects.filter(
                    research_folder=folder, collected__year=year, collected__month=month
                )
                .order_by("collected")
                .last()
            )
    ...
    for year in range(start_year, end_year + 1):
        ...
        for month in range(1, 13):
            if month not in data[year]:
                data[year][month] = []
            datasets = VaultDataset.objects.filter(
...
def _yearly_vault_stats(vault_folder, data, start_year, end_year):
    for year in range(start_year, end_year + 1):
        if year not in data:
            data[year] = {}
        datasets = VaultDataset.objects.filter(
        ...
        data[year]["num_datasets"] = datasets.count()
        data[year]["num_published"] = datasets.filter(status="PUBLISHED").count()
        s = (
            VaultStats.objects.filter(vault_folder=vault_folder, collected__year=year)
            .order_by("collected")
            .last()
        )
...
def calculate_yearly_cost(
    ...
    if size > first_block * GB:
        total_cost = (
            total_cost
            + (calculate_blocks(size - (first_block * GB), block_size)) * block_cost
        )
    return total_cost
...
def get_usage_data(start_year, end_year, include_revisions=True):
    ...
    for project in projects:
        rf = ResearchFolder.objects.filter(
            project=project
        )  # deleted folders should automatically disappear from the stats
        research = {}
        datasets = {}
        research_yearly = {}
        datasets_yearly = {}
        for f in rf:
            ...
            research = _monthly_research_stats(
                f, research, start_year, end_year, include_revisions=include_revisions
            )
            datasets = _vault_datasets_by_month(
        ...
        usage_data[project.id] = {
            ...
            "department": project.department.abbreviation,
    ...
    for project in data:
        ...
        if year in data[project]["research"]:
            ...
            for month in data[project]["datasets"][year]:
                ...
                for dataset in data[project]["datasets"][year][month]:
                    if "size" in dataset:
                        cost += (
                            calculate_yearly_cost(
                                dataset["size"], first_block_cost=25, block_cost=25
                            )
                            * dataset["retention"]
                        )
                        archive_cost += cost
            ...
            if "size" in data[project]["research_yearly"][year]:
                data[project]["calculated_by_last"] = calculate_yearly_cost(
                    data[project]["research_yearly"][year]["size"]
                )
...
def yearly_usage_formatted(year, data):
    ...
    for project in data:
        ...
        report_usage_data[row + 1][
            "project"
        ] = f'{data[project]["project"]}: active_cost'
        report_usage_data[row + 2]["project"] = f'{data[project]["project"]}: datasets'
        report_usage_data[row + 3][
            "project"
        ] = f'{data[project]["project"]}: archive_cost'
        for month in data[project]["research"][year]:
            if "size" in data[project]["research"][year][month]:
                report_usage_data[row][month] = round(
                    data[project]["research"][year][month]["size"] / GB
                )
                report_usage_data[row + 1][month] = round(
                    data[project]["research"][year][month]["cost"], 2
                )
                ...
                for dataset in data[project]["datasets"][year][month]:
                    report_usage_data[row + 2][
                        month
                    ] += f"{round(dataset['size'] / GB)} (retention: {dataset['retention']}), "
                    report_usage_data[row + 3][month] += round(dataset["cost"], 2)
                if report_usage_data[row + 2][month] == "":
                    report_usage_data[row + 2][month] = "-"
                else:
                    report_usage_data[row + 2][month] = report_usage_data[row + 2][
                        month
                    ][:-2]
        row += 4
...
def yearly_statistics_formatted(year, data):
    report_usage_data = {}
    row = 0
    for project in data:
        create_year = int(data[project]["created"][:4])
        if create_year <= year:
            if year in data[project]["research_yearly"]:
                report_usage_data[row] = {}
                report_usage_data[row]["project"] = data[project]["title"]
                report_usage_data[row]["faculty"] = data[project]["faculty"]
                report_usage_data[row]["department"] = data[project]["department"]
                report_usage_data[row]["created"] = data[project]["created"]
                report_usage_data[row]["deleted"] = data[project]["deleted"]
                report_usage_data[row]["active_size"] = round(
                    data[project]["research_yearly"][year]["size"] / GB, 1
                )
                report_usage_data[row]["archive_size"] = round(
                    data[project]["datasets_yearly"][year]["size"] / GB, 1
                )
                report_usage_data[row]["num_datasets"] = data[project][
                    "datasets_yearly"
                ][year]["num_datasets"]
...
def general_stats(start_year, end_year, include_revisions=True):
    stats = {}
    stats["categories"] = [
        "Projects",
        "Active",
...
def generate_statistics_report(include_revisions=True):
    ...
    for year in reversed(range(2022, today.year + 1)):
        project_data = yearly_statistics_formatted(year, usage_data)
        dfb = pd.DataFrame.from_dict(data=project_data, orient="index")
        dfb.to_excel(writer, sheet_name=f"projects {year}", index=False)
        for column in dfb:
            column_length = max(
                dfb[column].astype(str).map(len).max(), len(str(column))
            )
            col_idx = dfb.columns.get_loc(column)
            writer.sheets[f"projects {year}"].set_column(
